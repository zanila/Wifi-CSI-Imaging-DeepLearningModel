...
def get_person_mask_and_bbox(model, img_path: str, confidence_threshold: float):
    ...
    Returns the segmentation mask and bounding box for the highest-confidence
    'person' detection, or (None, None, None) if no person is found.
    """
...
def make_square_bbox(x1, y1, x2, y2, img_h, img_w, padding_ratio=0.1):
    """
    Expand a bounding box to a square with optional padding.
    The square is centered on the original bbox and clipped to image bounds.
    Args:
        x1, y1, x2, y2: original bounding box coordinates
        img_h, img_w: image dimensions for clipping
        padding_ratio: fractional padding to add (0.1 = 10% on each side)
    Returns:
        (x1_new, y1_new, x2_new, y2_new): square bounding box coordinates
    """
    bw = x2 - x1
    bh = y2 - y1
    cx = (x1 + x2) / 2.0
...
def process_single_image(model, img_path: str, mask_size: int,
    ...
    """
    Full pipeline for a single image: detect → crop → resize → binarize.
    Returns:
        mask_WxW: (W, W) uint8 binary mask, or None if no person detected
        metadata: dict with bbox, confidence, fg_ratio, etc.
    """
...
def create_qa_grid(image_dir: str, image_ids: list, masks: np.ndarray,
    ...
    """
    Create a visual QA grid showing: original image | 32×32 mask (upscaled).
...
def main():
    parser = argparse.ArgumentParser(
        description="Ground-Truth Extraction: RGB frames to binary silhouette masks"
    )
    parser.add_argument(
